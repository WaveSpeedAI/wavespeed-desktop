...
import { Search, FolderOpen, Play, Trash2, Pencil, Download, Upload, MoreVertical } from 'lucide-react'
    ...
    <div className="p-4 pb-20">
      ...
      <Dialog open={!!editingTemplate} onOpenChange={(open) => !open && setEditingTemplate(null)}>
        <DialogContent className="max-w-[90vw] sm:max-w-sm">
          ...
          <DialogFooter className="flex-row gap-2">
            ...
            <Button className="flex-1" onClick={handleSaveEdit} disabled={!editName.trim()}>
              {t('common.save')}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!deletingTemplate} onOpenChange={(open) => !open && setDeletingTemplate(null)}>
        <DialogContent className="max-w-[90vw] sm:max-w-sm">
          ...
          <DialogFooter className="flex-row gap-2">
            <Button variant="outline" className="flex-1" onClick={() => setDeletingTemplate(null)}>
              {t('common.cancel')}
            </Button>
            <Button variant="destructive" className="flex-1" onClick={handleDeleteTemplate}>
              {t('common.delete')}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!importData} onOpenChange={(open) => !open && setImportData(null)}>
        <DialogContent className="max-w-[90vw] sm:max-w-md">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <label className="flex items-start gap-3 p-3 border rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium text-sm">{t('templates.merge')}</div>
                ...
                <label className="flex items-start gap-3 p-3 border rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium text-sm">{t('templates.replaceAll')}</div>
                    <div className="text-xs text-muted-foreground">
          ...
          <DialogFooter className="flex-row gap-2">
            <Button variant="outline" className="flex-1" onClick={() => setImportData(null)}>
              {t('common.cancel')}
            </Button>
      ...
      <Dialog open={showBatchDeleteDialog} onOpenChange={setShowBatchDeleteDialog}>
        <DialogContent className="max-w-[90vw] sm:max-w-sm">
          ...
          <DialogFooter className="flex-row gap-2">
            <Button variant="outline" className="flex-1" onClick={() => setShowBatchDeleteDialog(false)}>
              {t('common.cancel')}
            </Button>
            <Button variant="destructive" className="flex-1" onClick={handleBatchDelete}>
              {t('common.delete')}
            </Button>
          </DialogFooter>
        </DialogContent>
