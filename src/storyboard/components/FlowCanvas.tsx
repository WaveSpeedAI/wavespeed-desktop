...
/* ── Strategy label config ─────────────────────────────── */

const STRATEGY_LABELS: Record<string, { text: string; color: string }> = {
  A1: { text: "续写", color: "text-blue-500 bg-blue-500/10 border-blue-500/20" },
  A2: { text: "切角", color: "text-violet-500 bg-violet-500/10 border-violet-500/20" },
  B:  { text: "长镜", color: "text-amber-500 bg-amber-500/10 border-amber-500/20" },
  C:  { text: "t2v",  color: "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" },
  D:  { text: "跨长", color: "text-rose-500 bg-rose-500/10 border-rose-500/20" },
};
...
export function FlowCanvas() {
  ...
  // Build structure: Act → Scene → Shots
  const structure = useMemo(() => {
    const acts = new Map<number, Map<string, typeof sorted>>();
    for (const shot of sorted) {
      if (!acts.has(shot.act_number)) acts.set(shot.act_number, new Map());
      const sceneMap = acts.get(shot.act_number)!;
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      >
        ...
        >
          {Array.from(structure.entries()).map(([actNum, sceneMap]) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 px-3 py-3 overflow-x-auto">
                        {sceneShots.map((shot, idx) => {
                          const nextShot = idx < sceneShots.length - 1 ? sceneShots[idx + 1] : null;
                          const nextStrategy = nextShot?.strategy;
                          // Edge style: thick solid = frame chain (A1), dashed = narrative only
                          const isFrameChain = nextStrategy?.use_frame_chain;
...
function ShotCard({
  ...
  shot: ReturnType<typeof useStoryboardStore.getState>["shots"][0];
  characters: ReturnType<typeof useStoryboardStore.getState>["characters"];
  isSelected: boolean;
  onClick: () => void;
  onRegenerate: () => void;
}) {
      ...
      >
        ...
        <div className="relative w-full aspect-video bg-muted/50 overflow-hidden">
          {shot.generated_assets.video_path ? (
            <video
              ...
              // Seek to 0.5s to get a representative frame as poster
        ...
        <div className="px-2 py-1.5" onClick={onClick}>
          <div className="flex items-center gap-1 mb-0.5">
            ...
            {shot.strategy && STRATEGY_LABELS[shot.strategy.strategy_type] && (
              <span className={cn(
                "text-[8px] font-mono px-1 py-0.5 rounded border shrink-0",
                STRATEGY_LABELS[shot.strategy.strategy_type].color,
              )}>
                {STRATEGY_LABELS[shot.strategy.strategy_type].text}
              </span>
            )}
          </div>
          {shotChars.length > 0 && (
          ...
          {shot.strategy?.is_scene_anchor_shot && (
            <span className="text-[8px] text-amber-600 dark:text-amber-400 font-mono">⚓ 锚定</span>
          )}
        </div>
      </div>

      {/* Video preview overlay */}
      {showOverlay && shot.generated_assets.video_path && (
        <ShotVideoOverlay
          videoUrl={shot.generated_assets.video_path}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b bg-card">
          <div className="flex items-center gap-2">
            ...
            {videoDuration > 0 && Math.abs(videoDuration - shotDuration) > 0.5 && (
              <span className="text-[9px] text-amber-600 dark:text-amber-400 font-mono">
                ⚠ 偏差 {formatTime(Math.abs(videoDuration - shotDuration))}
              </span>
            )}
          </div>
...
function TimelineRuler({
  ...
  const totalDuration = shots.reduce((sum, s) => sum + s.duration, 0) || 1;
      ...
      {shots.map((shot) => {
        ...
        return (
          <button
            ...
            title={`#${shot.sequence_number} · ${shot.duration}s`}
          />
        );
      })}
    </div>
