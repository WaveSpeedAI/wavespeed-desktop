  ...
  validate(params: Record<string, unknown>): ValidationResult {
    ...
    for (const paramDef of this.definition.params) {
      if (paramDef.validation) {
        ...
        if (paramDef.validation.min !== undefined && typeof val === 'number' && val < paramDef.validation.min) {
          errors.push(`${paramDef.label}: minimum is ${paramDef.validation.min}`)
        }
        if (paramDef.validation.max !== undefined && typeof val === 'number' && val > paramDef.validation.max) {
          errors.push(`${paramDef.label}: maximum is ${paramDef.validation.max}`)
        }
      }
    }
