...
export function registerStorageIpc(): void {
  ipcMain.handle('storage:get-workflow-snapshot', async (_event, args: { workflowId: string }) => {
    return getStorage().loadWorkflowSnapshot(args.workflowId)
  })

  ipcMain.handle('storage:get-execution-cache', async (_event, args: { workflowId: string; nodeId: string; executionId: string }) => {
    return null // simplified — execution cache not used in new structure
  })

  ipcMain.handle('storage:list-node-executions', async (_event, args: { workflowId: string; nodeId: string }) => {
    return getStorage().listNodeExecutions(args.workflowId, args.nodeId)
  })

  ipcMain.handle('storage:list-uploaded-files', async (_event, args: { workflowId: string; nodeId: string }) => {
    return getStorage().listUploadedFiles(args.workflowId, args.nodeId)
  })

  ipcMain.handle('storage:save-uploaded-file', async (_event, args: { workflowId: string; nodeId: string; filename: string; data: Buffer }) => {
  ...
  ipcMain.handle('storage:get-workflow-disk-usage', async (_event, args: { workflowId: string }) => {
    return getStorage().getWorkflowDiskUsage(args.workflowId)
  })

  ipcMain.handle('storage:delete-workflow-files', async (_event, args: { workflowId: string }) => {
    getStorage().deleteWorkflowFiles(args.workflowId)
  })

  ipcMain.handle('storage:artifact-exists', async (_event, args: { artifactPath: string }) => {
    return getStorage().artifactExists(args.artifactPath)
  })

  ipcMain.handle('storage:export-workflow-json', async (_event, args: { workflowId: string; workflowName: string; graphDefinition: unknown }) => {
    ...
    const data = { version: '1.0', id: args.workflowId, name: args.workflowName, exportedAt: new Date().toISOString(), graphDefinition: args.graphDefinition }
  ...
  ipcMain.handle('storage:open-workflow-folder', async (_event, args: { workflowId: string }) => {
    const workflowPath = getStorage().getWorkflowDir(args.workflowId)
    if (!existsSync(workflowPath)) mkdirSync(workflowPath, { recursive: true })
  ...
  ipcMain.handle('storage:import-workflow-json', async () => {
    ...
    try {
      ...
      const rawGraphDef = data.graphDefinition ?? (data.nodes ? { nodes: data.nodes, edges: data.edges ?? [] } : null)
      ...
      const enrichedNodes = rawGraphDef.nodes.map((n: Record<string, unknown>) => {
        const oldId = String(n.id)
        const newId = idMap.get(oldId)!
        const params = (n.params ?? {}) as Record<string, unknown>
        const nodeType = String(n.nodeType ?? 'ai-task/run')
        const position = (n.position as { x: number; y: number }) ?? { x: 200, y: 200 }
      ...
      const enrichedEdges = (rawGraphDef.edges ?? []).map((e: Record<string, unknown>) => ({
        id: uuid(),
        workflowId: wf.id,
  ...
  ipcMain.handle('storage:delete-node-outputs', async (_event, args: { workflowId: string; nodeId: string }) => {
    getStorage().deleteNodeOutputs(args.workflowId, args.nodeId)
  })

  ipcMain.handle('storage:clean-workflow-outputs', async (_event, args: { workflowId: string }) => {
