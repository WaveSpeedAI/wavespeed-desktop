...
export function CompInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  const { value: externalValue, onChange, onCompositionStart, onCompositionEnd, ...rest } = props
...
export function CompTextarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  const { value: externalValue, onChange, onCompositionStart, onCompositionEnd, ...rest } = props
  const composingRef = useRef(false)
  const [localValue, setLocalValue] = useState(String(externalValue ?? ''))
