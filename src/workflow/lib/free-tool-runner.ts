...
function float32ToImageData(data: Float32Array, width: number, height: number): ImageData {
...
export async function runImageEnhancer(
  ...
  const worker = new Worker(new URL('../../workers/upscaler.worker.ts', import.meta.url), { type: 'module' })

  return new Promise((resolve, reject) => {
    const handleMessage = (e: MessageEvent) => {
      const { type, payload } = e.data
      if (type === 'phase') {
        onProgress?.(15, payload?.phase === 'download' ? 'Downloading model...' : 'Processing...')
...
export async function runBackgroundRemover(
  ...
  const worker = new Worker(new URL('../../workers/backgroundRemover.worker.ts', import.meta.url), { type: 'module' })

  return new Promise((resolve, reject) => {
    const handleMessage = (e: MessageEvent) => {
      const { type, payload } = e.data
      if (type === 'phase') {
        onProgress?.(20, payload?.phase === 'download' ? 'Downloading model...' : 'Removing background...')
      } else if (type === 'progress') {
        const p = payload?.progress ?? 0
...
export async function runFaceEnhancer(
  ...
  const worker = new Worker(new URL('../../workers/faceEnhancer.worker.ts', import.meta.url), { type: 'module' })
    ...
    const handleMessage = (e: MessageEvent) => {
      ...
      if (type === 'phase') {
        ...
        worker.postMessage(
          {
            type: 'enhance',
            payload: { imageData: dataCopy, width: imageData.width, height: imageData.height, id: 0 }
          },
...
export async function runVideoEnhancer(
  ...
  const model = (params.model ?? 'slim') as ModelType
  const scale = (params.scale ?? '2x') as ScaleType
  if (!['slim', 'medium', 'thick'].includes(model)) throw new Error(`Invalid model: ${model}`)
  if (!['2x', '3x', '4x'].includes(scale)) throw new Error(`Invalid scale: ${scale}`)
  ...
  const upscalerWorker = new Worker(new URL('../../workers/upscaler.worker.ts', import.meta.url), { type: 'module' })
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    upscalerWorker.postMessage({ type: 'load', payload: { model, scale, id: 0 } })
  })
  ...
  const upscaleFrame = (imageData: ImageData): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      upscalerWorker.onmessage = handler
      upscalerWorker.postMessage({ type: 'upscale', payload: { imageData, id: 0 } })
    })
  }
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    const frame = new VideoFrame(outputCanvas, { timestamp: Math.round(t * 1_000_000) })
...
export async function runFaceSwapper(
  ...
  const worker = new Worker(new URL('../../workers/faceSwapper.worker.ts', import.meta.url), { type: 'module' })
  ...
  const detectFaces = (imageData: ImageData, imageId: 'source' | 'target'): Promise<Array<{ landmarks: number[][]; box: { x: number; y: number; width: number; height: number; confidence: number } }>> => {
    return new Promise((resolve, reject) => {
      ...
      worker.onmessage = handler
      worker.postMessage(
        { type: 'detect', payload: { imageData: float32, width: imageData.width, height: imageData.height, imageId, id } },
  ...
  const result = await new Promise<string>((resolve, reject) => {
    ...
    const handler = (e: MessageEvent) => {
      if (e.data.type === 'swapResult' && e.data.payload?.id === id) {
        ...
        const imgData = float32ToImageData(data instanceof Float32Array ? data : new Float32Array(data), width, height)
    ...
    worker.postMessage(
      {
        ...
        payload: {
          ...
          targetFaces: targetFaces.map(f => ({ landmarks: f.landmarks, box: f.box })),
          id
        }
      },
...
async function loadMaskAsFloat32(url: string, width: number, height: number): Promise<Float32Array> {
  const res = await fetch(url)
...
async function loadImageAsFloat32CHW(url: string): Promise<{ data: Float32Array; width: number; height: number }> {
...
function float32CHWToDataURL(data: Float32Array, width: number, height: number): string {
  const imageData = new ImageData(width, height)
  const out = imageData.data
  for (let i = 0; i < width * height; i++) {
    out[i * 4] = Math.round(Math.min(1, Math.max(0, data[i])) * 255)
    out[i * 4 + 1] = Math.round(Math.min(1, Math.max(0, data[width * height + i])) * 255)
    out[i * 4 + 2] = Math.round(Math.min(1, Math.max(0, data[2 * width * height + i])) * 255)
    out[i * 4 + 3] = 255
  }
  const canvas = document.createElement('canvas')
  canvas.width = width
...
export async function runImageEraser(
  ...
  const { data: imageData, width, height } = await loadImageAsFloat32CHW(imageUrl)
  const maskData = await loadMaskAsFloat32(maskUrl, width, height)

  const worker = new Worker(new URL('../../workers/imageEraser.worker.ts', import.meta.url), { type: 'module' })
  ...
  return new Promise((resolve, reject) => {
    const handleMessage = (e: MessageEvent) => {
      ...
      if (type === 'phase') {
        ...
        worker.postMessage(
          { type: 'process', payload: { imageData: imgCopy, maskData: maskCopy, width, height, id: 0 } },
          { transfer: [imgCopy.buffer, maskCopy.buffer] }
        )
      } else if (type === 'result') {
        const { data, width: w, height: h } = payload
        const result = data instanceof Float32Array ? data : new Float32Array(data)
...
export async function runSegmentAnything(
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      points = parsed.filter(
        (p: unknown): p is SegmentPointInput =>
          ...
          ((p as SegmentPointInput).label === 0 || (p as SegmentPointInput).label === 1)
      )
    } else {
      points = []
    }
  ...
  const worker = new Worker(new URL('../../workers/segmentAnything.worker.ts', import.meta.url), { type: 'module' })
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    worker.onmessage = handler
    worker.postMessage({ type: 'segment', payload: { id: 0, imageDataUrl: imageUrl } })
  })

  onProgress?.(60, 'Decoding mask...')
  const decodeId = 1
  const pointsForWorker = points.map(p => ({ point: p.point as [number, number], label: p.label as 0 | 1 }))
  const maskResult = await new Promise<{ mask: Uint8Array; width: number; height: number }>((resolve, reject) => {
    const handler = (e: MessageEvent) => {
