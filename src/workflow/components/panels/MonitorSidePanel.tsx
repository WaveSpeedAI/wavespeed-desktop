...
export function MonitorSidePanel({ workflowId }: { workflowId?: string | null }) {
  ...
  const onResizeStart = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => setSidebarWidth(startWidth + (startX - ev.clientX))
    const onUp = () => {
      setDragging(false)
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 h-12 border-b border-border/70 shrink-0">
        <span className="font-semibold text-[13px] text-foreground">History</span>
        <button
          onClick={togglePanel}
...
function SessionCard({ session, nodeStatuses, progressMap, errorMessages, onCancel }: {
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-border/40 bg-card mb-0.5">
      <div className="flex items-center gap-1.5 px-2.5 py-1.5 cursor-pointer hover:bg-muted/60 transition-colors"
      ...
      <div className="flex items-center gap-2 px-2.5 pb-1.5">
        <span className={cn('text-[9px] font-medium', statusColor)}>{statusLabel}</span>
        <span className="text-[9px] text-muted-foreground/50">{completed + errors}/{total}</span>
