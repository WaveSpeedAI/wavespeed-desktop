...
import { getBezierPath, type EdgeProps, EdgeLabelRenderer, BaseEdge } from 'reactflow'
...
export function CustomEdge({
  ...
  sourcePosition, targetPosition, style = {}, selected, source, target: _target
}: EdgeProps) {
  const status = useExecutionStore(s => s.edgeStatuses[id] ?? 'no-data') as EdgeStatus
  const sourceNodeStatus = useExecutionStore(s => s.nodeStatuses[source] ?? 'idle')
  ...
  else if (sourceNodeStatus === 'confirmed' && status === 'has-data') edgeColor = '#4ade80'
  ...
  return (
    <>
      <BaseEdge
        ...
        style={{
          ...style,
          stroke: selected ? 'hsl(var(--primary))' : isHovered ? 'hsl(var(--muted-foreground))' : edgeColor,
