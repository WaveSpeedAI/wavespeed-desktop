...
function useOrderedResultNodes(): Array<{ nodeId: string; label: string; status: string; urls: string[] }> {
  ...
  return useMemo(() => {
    ...
    for (const n of nodes) {
      ...
      if (nodeType === 'ai-task/run' && modelId) {
        ...
        idToDisplayLabel[n.id] = (rawTitle && rawTitle !== modelId) ? `${rawTitle} · ${modelId}` : modelId
      } else {
    ...
    if (latestSession?.nodeIds?.length) {
      ...
      const simpleEdges = edges.map(e => ({ sourceNodeId: e.source, targetNodeId: e.target }))
...
export function WorkflowResultsPanel() {
  ...
  const setWorkflowResultsPanelWidth = useUIStore(s => s.setWorkflowResultsPanelWidth)
  const toggleWorkflowResultsPanel = useUIStore(s => s.toggleWorkflowResultsPanel)
        ...
        <div className="p-2 space-y-2">
          {ordered.length === 0 && (
            <p className="text-muted-foreground text-sm py-6 text-center px-2">
              {t('workflow.noResultsYet', 'No results yet. Run the workflow to see outputs here.')}
            </p>
          )}
          {ordered.map(({ nodeId, label, status, urls }) => (
            ...
            >
              ...
              >
                <span className="text-xs font-medium text-foreground min-w-0 break-words">{label}</span>
                <span
                ...
                >
                  {status === 'running' ? t('workflow.running', 'Running') :
                    status === 'confirmed' ? t('workflow.done', 'Done') :
                    status === 'error' ? t('workflow.error', 'Error') : t('workflow.idle', 'Idle')}
                </span>
              </button>
              {urls.length > 0 && (
                <div className="px-2 pb-2 flex gap-1.5 flex-wrap">
                  {urls.slice(0, 3).map((url, ui) => {
                    ...
                    if (type === 'image') {
                      return (
                        <button
                          key={ui}
                          type="button"
                          onClick={e => { e.stopPropagation(); openPreview(url, urls) }}
                    ...
                    if (type === 'text') {
                      const text = url.startsWith('data:text/') ? decodeDataText(url) : url
                    ...
                    return (
                      ...
                      >
                        {url.startsWith('data:') ? 'Data' : (url.split('/').pop()?.split('?')[0] || 'File')}
                      </button>
                    )
                  })}
                  {urls.length > 3 && (
                    <span className="text-[10px] text-muted-foreground self-center">+{urls.length - 3}</span>
                  )}
                </div>
              )}
            </div>
