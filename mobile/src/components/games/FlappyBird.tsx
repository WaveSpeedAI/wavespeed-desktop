  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      if (gameStateRef.current === 'playing') {
        ...
        if (birdRef.current.y < 0 || birdRef.current.y + BIRD_SIZE > height - 40) {
          gameStateRef.current = 'gameover'
          setGameState('gameover')
          // Update high score
          if (scoreRef.current > highScore) {
            setHighScore(scoreRef.current)
            localStorage.setItem('flappybird_highscore', scoreRef.current.toString())
          }
        }
        ...
        pipesRef.current = pipesRef.current.filter(pipe => {
          ...
          if (birdRight > pipe.x && BIRD_X < pipeRight) {
            if (birdRef.current.y < pipe.topHeight || birdBottom > bottomPipeTop) {
              gameStateRef.current = 'gameover'
              setGameState('gameover')
              if (scoreRef.current > highScore) {
                setHighScore(scoreRef.current)
      ...
      ctx.beginPath()
      ctx.ellipse(BIRD_X + BIRD_SIZE/2, birdY + BIRD_SIZE/2, BIRD_SIZE/2, BIRD_SIZE/2.5, 0, 0, Math.PI * 2)
      ctx.fill()
      // Wing
      ctx.fillStyle = '#f59e0b'
      ctx.beginPath()
      ctx.ellipse(BIRD_X + BIRD_SIZE/3, birdY + BIRD_SIZE/2 + 2, BIRD_SIZE/4, BIRD_SIZE/5, 0, 0, Math.PI * 2)
      ctx.fill()
      // Eye
      ctx.fillStyle = 'white'
      ctx.beginPath()
      ctx.arc(BIRD_X + BIRD_SIZE/2 + 4, birdY + BIRD_SIZE/3, 5, 0, Math.PI * 2)
      ctx.fill()
      ctx.fillStyle = 'black'
      ctx.beginPath()
      ctx.arc(BIRD_X + BIRD_SIZE/2 + 5, birdY + BIRD_SIZE/3, 2.5, 0, Math.PI * 2)
      ctx.fill()
      // Beak
      ...
      if (gameStateRef.current === 'gameover') {
        ...
        ctx.fillStyle = '#94a3b8'
        ctx.fillText(`High Score: ${Math.max(scoreRef.current, highScore)}`, width / 2, height / 2 + 35)
        ctx.fillStyle = 'white'
        ctx.font = '16px sans-serif'
