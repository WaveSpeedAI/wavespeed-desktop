...
function useOptimizerFields() {
  ...
  const textField = useMemo(() => allFields.find(f => f.name === 'text'), [allFields])
...
export function WorkflowPromptOptimizer({
  ...
  const { optimizerModel, quickFields, textField, defaults } = useOptimizerFields()
  ...
  return (
    <div className="relative inline-flex" ref={containerRef} onClick={e => e.stopPropagation()}>
      ...
      {/* ── Error Toast ────────────────────────────────────── */}
      {error && (
        <div className="absolute top-7 left-0 z-[101] w-48 px-2.5 py-1.5 rounded-md border border-red-500/30 bg-[hsl(var(--popover))] text-[10px] text-red-400 shadow-lg cursor-pointer"
      ...
      {/* ── Dropdown Panel ─────────────────────────────────── */}
      {dropdownOpen && (
        <div className="absolute top-7 left-0 z-[100] w-56 rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--popover))] text-[hsl(var(--popover-foreground))] shadow-xl"
          onClick={e => e.stopPropagation()}>
          ...
          <div className="px-3 pb-2">
            ...
            {textField?.description && (
              <p className="mt-0.5 text-[9px] text-[hsl(var(--muted-foreground))] leading-tight">{textField.description}</p>
            )}
          </div>
      ...
      {imageDialogOpen && (
        <ImageOptimizeDialog
          ...
          onOptimized={result => { onOptimized(result); setImageDialogOpen(false) }}
...
function QuickField({ field, value, onChange }: {
  field: FormFieldConfig; value: unknown; onChange: (v: unknown) => void
}) {
  ...
  if (hasOptions && field.options!.length <= 4) {
    return (
      <div className="flex items-center justify-between gap-2">
        <span className="text-[11px] text-[hsl(var(--muted-foreground))] flex-shrink-0">{field.label || field.name}</span>
        <PillToggle
  ...
  if (field.type === 'boolean') {
    return (
      <div className="flex items-center justify-between gap-2">
        <span className="text-[11px] text-[hsl(var(--muted-foreground))]">{field.label || field.name}</span>
        <button type="button" onClick={() => onChange(!cur)}
          ...
          <span className={`pointer-events-none inline-block h-3 w-3 rounded-full bg-white shadow transform transition-transform ${cur ? 'translate-x-3' : 'translate-x-0'}`} />
        </button>
      </div>
    )
  }
  ...
  if (field.type === 'number' || field.type === 'slider') {
    return (
      <div className="flex items-center justify-between gap-2">
        <span className="text-[11px] text-[hsl(var(--muted-foreground))] flex-shrink-0">{field.label || field.name}</span>
        <input type="number" value={cur !== undefined && cur !== null ? Number(cur) : ''}
...
function ImageOptimizeDialog({ currentPrompt, defaultMode, onOptimized, onClose }: {
  currentPrompt: string
  defaultMode: string
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative w-full max-w-md mx-4 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--card-foreground))] shadow-2xl"
        onClick={e => e.stopPropagation()}>
...
function SpinnerIcon({ size = 12, className = '' }: { size?: number; className?: string }) {
  return (
