...
interface WorkerMessage {
  type: 'phase' | 'progress' | 'ready' | 'segmented' | 'maskResult' | 'reset' | 'error' | 'disposed'
  payload: PhasePayload | ProgressPayload | MaskResultPayload | ErrorPayload | { id: number }
}

export function useSegmentAnythingWorker(options: UseSegmentAnythingOptions = {}) {
  const workerRef = useRef<Worker | null>(null)
  const optionsRef = useRef(options)
  const idCounterRef = useRef(0)
  const maskCallbacksRef = useRef<Map<number, (result: MaskResult) => void>>(new Map())
  ...
  const createWorker = useCallback(() => {
    ...
    workerRef.current.onmessage = (e: MessageEvent<WorkerMessage>) => {
      ...
      switch (type) {
        ...
        case 'maskResult': {
          const { mask, width, height, scores, id } = payload as MaskResultPayload
