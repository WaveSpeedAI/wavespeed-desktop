...
export function DynamicForm({
  ...
  const renderField = (field: FormFieldConfig, index?: number) => {
    const animStyle = index !== undefined ? { animationDelay: `${index * 50}ms` } : undefined

    // Hidden fields render with a toggle
    if (field.hidden) {
      const isEnabled = enabledHiddenFields.has(field.name)
      return (
        <div key={field.name} className={cn("space-y-2", collapsible && "field-animate")} style={animStyle}>
          <div className="space-y-1">
            ...
            >
              <div className={cn(
                ...
                isEnabled ? "bg-primary-foreground border-primary-foreground scale-110" : "border-muted-foreground"
              )} />
              {field.label}
            </button>
    ...
    if (collapsible) {
      return (
        <div key={field.name} className={cn("field-hover", animStyle && "field-animate")} style={animStyle}>
          <FormField
            field={field}
            value={values[field.name]}
            onChange={(value) => onChange(field.name, value)}
          ...
          <div className={cn("grid-collapse", showAdvanced && "open")}>
            <div>
              <div className="space-y-4 rounded-lg bg-muted/30 p-3 -mx-1 border border-border/40">
                {showAdvanced && advancedFields.map((field, i) => renderField(field, i))}
              </div>
            </div>
          </div>
        </>
