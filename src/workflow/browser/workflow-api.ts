...
import type { CreateWorkflowInput, SaveWorkflowInput } from '@/workflow/types/ipc'
...
const NOT_AVAILABLE = 'Not available in browser (run in Electron for full workflow features).'
...
async function handleInvoke(channel: string, args?: unknown): Promise<unknown> {
  switch (channel) {
    ...
    case 'workflow:save': {
      ...
      storage.updateWorkflow(input.id, input.name, { nodes: input.nodes, edges: input.edges }, input.status)
      return undefined
    }
    case 'workflow:load': {
    ...
    case 'settings:get-api-keys':
      try {
        const raw = typeof window !== 'undefined' ? localStorage.getItem('wavespeed_api_key') : null
    ...
    default:
      if (channel.startsWith('storage:') || channel.startsWith('upload:') || channel.startsWith('free-tool:')) {
