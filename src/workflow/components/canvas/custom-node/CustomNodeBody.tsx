...
import { type CustomNodeData, ML_FREE_TOOLS, paramDefToFormFieldConfig, portToFormFieldConfig } from './CustomNodeTypes'
import { HandleAnchor } from './CustomNodeHandleAnchor'
import { Row, LinkedBadge, ConnectedInputControl, Tip, Inline3DViewer } from './CustomNodePrimitives'
import { ParamRow, MediaRow, LoraRow, JsonRow, DefParamControl, InputPortControl } from './CustomNodeParamControls'
...
export interface CustomNodeBodyProps {
  ...
  localizeParamDescription: (key: string, fallback?: string) => string | undefined
...
export function CustomNodeBody(props: CustomNodeBodyProps) {
  const {
    id, data, status, edges, connectedSet, inputDefs, paramDefs,
    isAITask, isPreviewNode, currentModelId, currentModel,
    storeModels, getModelById,
    usePlaygroundForm, visibleFormFields, hiddenFormFields,
    enabledHiddenFields, setEnabledHiddenFields,
    orderedVisibleParams, optionalParams, showOptional, setShowOptional,
    formValues, resultGroups,
    setParam, updateNodeParams, openPreview,
    handleInlineSelectModel, handleWorkflowUploadFile,
    localizeInputLabel, localizeParamLabel, localizeParamDescription,
    segmentPointPickerOpen, setSegmentPointPickerOpen, ensureWorkflowId,
    inlineInputPreviewUrl, inlinePreviewIsImage, inlinePreviewIsVideo,
    inlinePreviewIsAudio, inlinePreviewIs3D,
  ...
  return (
    <div className="px-1 py-2 space-y-px">
      ...
      {status === 'idle' && resultGroups.length === 0 && ML_FREE_TOOLS.has(data.nodeType) && (
      ...
      {/* defParams */}
      {data.nodeType !== 'input/media-upload' && data.nodeType !== 'input/text-input' && paramDefs.map(p => {
