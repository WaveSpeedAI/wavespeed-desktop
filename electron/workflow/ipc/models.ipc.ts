...
import { getModels, searchModels, getModelById, syncModelsFromRenderer } from '../services/model-list'
...
export function registerModelsIpc(): void {
   ...
   */
  ipcMain.handle('models:sync', async (_event, models: Model[]): Promise<void> => {
  ...
  ipcMain.handle('models:search', async (
    _event, args: { query: string; category?: string; provider?: string }
  ): Promise<WaveSpeedModel[]> => {
    return searchModels(args.query, { category: args.category, provider: args.provider })
  })

  ipcMain.handle('models:get-schema', async (
    _event, args: { modelId: string }
