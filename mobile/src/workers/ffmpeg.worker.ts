...
async function ensureLoaded(
  ...
  loadingPromise = (async () => {
    ...
    const urls = getFFmpegURLs()
    console.log('[FFmpeg] Using single-threaded version for browser compatibility')
    ...
    console.log('[FFmpeg] Loading FFmpeg using toBlobURL (uses browser HTTP cache)...')
    ...
    try {
      ...
      onProgress?.(40)
      console.log('[FFmpeg] Fetching ffmpeg-core.wasm (~24MB, uses HTTP cache if available)...')
      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(() => reject(new Error('FFmpeg initialization timed out (60s). Try refreshing the page.')), 60000)
        })
...
function buildConvertArgs(
  ...
  console.log(`[FFmpeg] Building args for format: ${outputFormat}, options:`, options)
  ...
  try {
    switch (type) {
      ...
      case 'convert': {
        ...
        const args = buildConvertArgs(fileName, outputFile, outputFormat, options)
