/**
 * Left panel — Character cards, Scene cards, Global style settings (v3.0).
 */
import { useStoryboardStore } from "../stores/storyboard.store";
import { ScrollArea } from "@/components/ui/scroll-area";
import { cn } from "@/lib/utils";
import { User, MapPin, Palette, Pencil } from "lucide-react";

export function LeftPanel() {
  const characters = useStoryboardStore((s) => s.characters);
  const scenes = useStoryboardStore((s) => s.scenes);
  const superDID = useStoryboardStore((s) => s.superDID);
  ...
  return (
    <ScrollArea className="flex-1">
      <div className="p-3 space-y-4">
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <div
                      ...
                      style={!char.turnaround_image ? { backgroundColor: `hsl(${hue}, 50%, 85%)` } : undefined}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-muted-foreground line-clamp-2 leading-tight">
                        {char.immutable_traits.face_description}
                      </p>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2">
                    <div
                      className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 overflow-hidden"
                      style={!scene.master_frame ? { backgroundColor: `hsl(${hue}, 45%, 88%)` } : undefined}
                    >
                      {scene.master_frame ? (
        ...
        {/* Global Style (from SuperDID cinematic identity) */}
        {superDID && (
          <section>
            <h3 className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 mb-2 flex items-center gap-1">
            ...
            <div className="rounded-lg border border-border/50 p-2 space-y-1 text-[10px] text-muted-foreground">
              <p>风格: {superDID.cinematic_identity.art_style}</p>
              <p>色调: {superDID.cinematic_identity.color_palette.join(", ")}</p>
              <p>氛围: {superDID.cinematic_identity.visual_mood}</p>
              <p>时长: {superDID.target_duration}s ({superDID.duration_type})</p>
            </div>
          </section>
        )}
      </div>
