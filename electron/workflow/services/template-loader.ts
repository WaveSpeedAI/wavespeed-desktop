...
function loadI18nSearchMap(): Record<string, string> {
  ...
  try {
    ...
    console.log(`[TemplateLoader] Built i18n search index for ${Object.keys(map).length} preset keys from ${localeFiles.length} locales`)
  } catch (error) {
...
export function loadTemplatesFromFiles(): CreateTemplateInput[] {
  ...
  if (!fs.existsSync(templatesDir)) {
    console.warn('[TemplateLoader] Templates directory not found:', templatesDir)
    return templates
  }
        ...
        try {
          ...
          if (!thumbnail) {
            const imageUploadNode = cleanedNodes.find(
              (n: any) => n.nodeType === 'input/media-upload' && n.params?.mediaType === 'image' && n.params?.uploadedUrl
            )
            if (imageUploadNode) {
          ...
          console.log(`[TemplateLoader] Loaded template: ${templateName} from ${categoryDir}/${file}`)
        } catch (error) {
          console.error(`[TemplateLoader] Failed to load template from ${categoryDir}/${file}:`, error)
        }
      }
    }

    console.log(`[TemplateLoader] Successfully loaded ${templates.length} templates from files`)
  } catch (error) {
