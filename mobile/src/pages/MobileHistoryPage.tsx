...
function ProxyImage({ src, alt, className }: { src: string; alt: string; className?: string }) {
...
export function MobileHistoryPage() {
  ...
  const { get: getLocalInputs, getArchived, load: loadPredictionInputs, isLoaded: inputsLoaded } = usePredictionInputsStore()
  ...
  const getItemInputs = (item: ExtendedHistoryItem): { inputs: Record<string, unknown>; modelName: string } | null => {
    console.log('[MobileHistoryPage] Looking up inputs for prediction:', item.id)
    ...
    if (localEntry && localEntry.inputs && Object.keys(localEntry.inputs).length > 0) {
    ...
    if (item.inputs && typeof item.inputs === 'object' && Object.keys(item.inputs).length > 0) {
      console.log('[MobileHistoryPage] Found inputs in API response (inputs field)')
      return { inputs: item.inputs, modelName: item.model }
    }
    if (item.input && typeof item.input === 'object' && Object.keys(item.input).length > 0) {
      console.log('[MobileHistoryPage] Found inputs in API response (input field)')
  ...
  const handleSaveAsTemplate = async () => {
    ...
    if (!inputData) {
      ...
      try {
        ...
        const apiInputs = (details as { input?: Record<string, unknown> }).input
        if (apiInputs && typeof apiInputs === 'object' && Object.keys(apiInputs).length > 0) {
          ...
          setSelectedItem(prev => prev ? { ...prev, input: apiInputs } : null)
        }
      } catch (err) {
    ...
    const modelName = inputData.modelName.split('/').pop() || inputData.modelName
                    ...
                    <div className="aspect-square bg-muted relative">
                      ...
                      <div className="absolute top-1.5 right-1.5 flex items-center gap-1">
                        ...
                        {inputsLoaded && hasInputsAvailable(item) && (
                          <div className="bg-primary/90 rounded-full p-1" title={t('history.hasInputs')}>
                            <BookmarkPlus className="h-3 w-3 text-primary-foreground" />
                          </div>
                        )}
                        {getStatusBadge(item.status, statusFilter === 'archived')}
                      </div>
                    </div>

                    <CardContent className="p-2">
                      <p className="font-medium text-xs truncate">{item.model.split('/').pop()}</p>
          ...
          {selectedItem && (
            <div className="flex-1 overflow-y-auto space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <p className="text-xs text-muted-foreground">{t('history.model')}</p>
                  <p className="font-medium text-xs truncate">{selectedItem.model}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">{t('history.status.all').replace('All ', '')}</p>
                  <div>{getStatusBadge(selectedItem.status)}</div>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">{t('history.created')}</p>
                  <p className="font-medium text-xs">{formatDate(selectedItem.created_at)}</p>
                </div>
                {selectedItem.execution_time && (
                  <div>
                    <p className="text-xs text-muted-foreground">{t('history.executionTime')}</p>
                    <p className="font-medium text-xs">{(selectedItem.execution_time / 1000).toFixed(2)}s</p>
                  </div>
                )}
                <div className="col-span-2">
                  <p className="text-xs text-muted-foreground">{t('history.predictionId')}</p>
                  <div className="flex items-center gap-2">
              ...
              {selectedItem.status === 'completed' && (
                ...
                >
                  ...
                  {isFetchingInputs ? t('common.loading') : t('history.saveTemplate.button')}
                </Button>
              )}
            </div>
          )}
        ...
        <DialogContent className="max-w-[90vw]">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="template-name">{t('history.saveTemplate.nameLabel')}</Label>
              <Input
                id="template-name"
                value={templateName}
                onChange={(e) => {
