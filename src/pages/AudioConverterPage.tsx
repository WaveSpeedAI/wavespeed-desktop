...
export function AudioConverterPage() {
  ...
  const handleFileSelect = useCallback(
    (file: File) => {
      if (!file.type.startsWith('audio/') && !file.name.match(/\.(mp3|m4a|ogg|wav|flac|aac|wma)$/i)) {
        return
      }
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const result = await convert(arrayBuffer, audioFile.name, outputFormat, format.ext, {
  ...
  return (
    ...
    >
      ...
      {isDragging && audioFile && (
        <div className="absolute inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center border-2 border-dashed border-primary rounded-lg m-4">
          <div className="text-center">
            ...
            <p className="text-lg font-medium">{t('freeTools.audioConverter.dropToReplace')}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold">{t('freeTools.audioConverter.title')}</h1>
      ...
      {!audioFile && (
        ...
        >
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-lg font-medium">{t('freeTools.audioConverter.selectAudio')}</p>
      ...
      {audioFile && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            {selectedFormat && selectedFormat.id !== 'flac' && selectedFormat.id !== 'wav' && (
                ...
                {convertedUrl ? (
                  ...
                  <div className="h-[54px] bg-muted rounded-lg flex items-center justify-center">
                    <div className="text-center text-muted-foreground text-sm">
                      ...
                      <span>{selectedFormat?.label} • {bitrate}</span>
                    </div>
                  </div>
                )}
              </CardContent>
