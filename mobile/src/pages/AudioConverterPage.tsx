...
export function AudioConverterPage() {
  ...
  const handleFileSelect = useCallback((file: File) => {
    ...
    if (!file.name.match(/\.(mp3|wav)$/i)) {
      setError(t('freeTools.audioConverter.invalidFile', 'Please select an MP3 or WAV file'))
      return
    }
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const result = await convert(arrayBuffer, audioFile.name, outputFormat, bitrate)
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold">{t('freeTools.audioConverter.title')}</h1>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            {t('freeTools.audioConverter.supportedFormats', 'MP3 ↔ WAV conversion')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!audioFile ? (
            ...
            >
              ...
              <p className="text-sm text-muted-foreground mb-1">
                {t('freeTools.audioConverter.selectAudio', 'Click or drag MP3/WAV file here')}
              </p>
            </div>
          ) : (
      ...
      {audioFile && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">{t('common.settings', 'Settings')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-xs font-medium">{t('freeTools.audioConverter.outputFormat', 'Output Format')}</label>
                <Select value={outputFormat} onValueChange={(v) => setOutputFormat(v as 'mp3' | 'wav')} disabled={isProcessing}>
              ...
              {outputFormat === 'mp3' && (
                <div className="space-y-1.5">
                  <label className="text-xs font-medium">{t('freeTools.audioConverter.bitrate', 'Bitrate')}</label>
                  <Select value={String(bitrate)} onValueChange={(v) => setBitrate(Number(v))} disabled={isProcessing}>
                    <SelectTrigger className="h-9">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
