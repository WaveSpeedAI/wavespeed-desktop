...
const pending = new Map<string, { resolve: (r: NodeExecutionResult) => void; reject: (e: Error) => void; startTime: number }>()
...
export function registerFreeToolIpc(): void {
  ipcMain.handle('free-tool:complete', async (_event, payload: FreeToolCompletePayload) => {
    const { requestId, workflowId, nodeId, outputData, outputExt, outputPrefix } = payload
    ...
    try {
      ...
      const filePath = storage.saveNodeOutput(workflowId, nodeId, outputPrefix, outputExt, buffer)
  ...
  ipcMain.handle('free-tool:error', async (_event, payload: FreeToolErrorPayload) => {
 ...
 */
export function executeFreeToolInRenderer(req: Omit<FreeToolExecuteRequest, 'requestId'>): Promise<NodeExecutionResult> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      if (entry) {
        pending.delete(requestId)
        entry.reject(new Error(`Free-tool execution timed out (${timeoutMs / 1000}s)`))
      }
    }, timeoutMs)
    ...
    pending.set(requestId, { resolve: wrapResolve, reject: wrapReject, startTime })
