...
import type { Template, TemplateFilter, CreateTemplateInput } from '../../../src/types/template'
...
export function createTemplate(input: CreateTemplateInput): Template {
  ...
  const playgroundData = input.templateType === 'playground' && input.playgroundData
    ? JSON.stringify(input.playgroundData) 
    : null
  const workflowData = input.templateType === 'workflow' && input.workflowData
  ...
  db.run(
    ...
    [
      id, input.name, input.description || null, tags, input.type,
      input.templateType, 0, now, now, input.author || null, 0,
      input.thumbnail || null, playgroundData, workflowData
    ]
  )
...
export function queryTemplates(filter?: TemplateFilter): Template[] {
  ...
  const orderBy = filter?.sortBy === 'useCount' 
...
export function toggleFavorite(id: string): boolean {
  ...
  db.run('UPDATE templates SET is_favorite = ?, updated_at = ? WHERE id = ?',
    [newValue, new Date().toISOString(), id])
  persistDatabase()
  return newValue === 1
}
...
export function incrementUseCount(id: string): void {
  const db = getDatabase()
  db.run('UPDATE templates SET use_count = use_count + 1, updated_at = ? WHERE id = ?',
