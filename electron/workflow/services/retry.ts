...
const DEFAULT_CONFIG: RetryConfig = { maxRetries: 3, baseDelayMs: 1000, skipClientErrors: true }
...
export async function withRetry<T>(
  ...
  while (true) {
    ...
    try {
      ...
      if (error instanceof ApiError && cfg.skipClientErrors && error.statusCode >= 400 && error.statusCode < 500) {
        throw error
      }
