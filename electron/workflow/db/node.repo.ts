...
export function getNodesByWorkflowId(workflowId: string): WorkflowNode[] {
  ...
  return result[0].values.map(row => ({
    id: row[0] as string, workflowId: row[1] as string, nodeType: row[2] as string,
    position: { x: row[3] as number, y: row[4] as number },
    params: JSON.parse(row[5] as string), currentOutputId: row[6] as string | null
  }))
}

export function updateNodeParams(nodeId: string, params: Record<string, unknown>): void {
  const db = getDatabase()
  db.run('UPDATE nodes SET params = ? WHERE id = ?', [JSON.stringify(params), nodeId])
  persistDatabase()
}

export function updateNodeCurrentOutputId(nodeId: string, executionId: string | null): void {
  const db = getDatabase()
  db.run('UPDATE nodes SET current_output_id = ? WHERE id = ?', [executionId, nodeId])
  persistDatabase()
}
